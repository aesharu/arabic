...
- **But the pronunciation is in Ukrainian letters as well** (23 Sept 2026, my decision — I tried it with Dima and it works far better than English spelling): under every Latin pronunciation the site shows the same sounds in Ukrainian, worked out by `public/js/core/ua.js` and locked down by `tests/ua.test.mjs`. ق = **ґ** (as in «ґанок»), خ and ح = **х**, ه and غ = **г**, ل before a consonant = **ль**. It's an extra line, never a replacement, and only in my profile — Dima is the native speaker and doesn't need it. Any single word's Ukrainian can be corrected in the ✎ form ("How to say it, in Ukrainian letters"); the correction wins over the worked-out one.

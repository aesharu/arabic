...
import { esc, plain, arabicSpans, safeHtml, PLAY_ICON, SLOW_ICON } from "./util.js";
...
export function playBtn(note, kind, label, { voice = "", rate = "", cls = "" } = {}) {
  ...
  return `<button class="play ${cls}${rate ? " slow" : ""}" data-play="${kind}" data-guid="${esc(note.guid)}"${voice ? ` data-voice="${voice}"` : ""}${rate ? ` data-rate="${rate}"` : ""}
    title="${rate ? "Slow" : "Play"} ${kind === "word" ? "word" : "sentence"}">${icon}${label ? `<span>${esc(label)}</span>` : ""}</button>`;
}
...
export function example(note, { audio = true } = {}) {
  ...
  return `<div class="ex"><div class="ar">${esc(ar)}</div>
    ...
    ${audio && hasAudio(note, "sent") ? `<div class="plays">${playBtn(note, "sent", "Sentence")}${playBtn(note, "sent", "♂", { voice: "m", cls: "icon" })}${playBtn(note, "sent", "♀", { voice: "f", cls: "icon" })}${playBtn(note, "sent", "", { rate: 0.7, cls: "icon" })}</div>` : ""}</div>`;
}

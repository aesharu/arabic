// "Talk about yourself" (core/aboutme.js, data/me.js): every choice in both languages, and whatever you choose,
// every sentence comes out in Saudi, pronunciation and English.
...
const check = (l, where) => {
  for (const f of ["ar", "say", "en"]) assert.ok(l[f], `${where}: ${f}`);
  assert.match(l.ar, /[؀-ۿ]/, `${where}: ${l.ar}`);
  assert.doesNotMatch(l.say, /[؀-ۿ]/, `${where}: ${l.say}`);
  assert.doesNotMatch(`${l.ar} ${l.say} ${l.en}`, /undefined|null|NaN/, `${where}: ${l.en}`);
};

test("every choice: Saudi, pronunciation and English", () => {
...
test("whatever you choose, every sentence is complete", () => {
  const cases = [
    {},
    { age: 28, city: "kyiv", country: "poland", langs: ["uk", "en"], hobbies: ["football", "travel", "cooking"], brothers: 1, sisters: 2 },

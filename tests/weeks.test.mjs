// Weekly lessons (public/js/data/weeks.js): weeks 3–67, real topics and grammar, conversations in four languages.
...
test("speaking tasks and conversations are in four languages", () => {
  for (const x of [...Object.values(TASKS).flat()]) for (const l of ["en", "uk", "najdi", "msa"]) assert.ok(x[l], `task ${x.en}: ${l}`);
  ...
  for (const [w, lines] of Object.entries(DIALOGUES)) {
    for (const l of lines) {
      for (const f of ["who", "ar", "say", "en", "uk", "msa"]) assert.ok(l[f], `week ${w} ${l.ar}: ${f}`);
      assert.match(l.uk, /[Ѐ-ӿ]/, `week ${w} ${l.ar}: Ukrainian should be Cyrillic`);
      assert.match(l.msa, /[؀-ۿ]/, `week ${w} ${l.ar}: MSA should be Arabic`);

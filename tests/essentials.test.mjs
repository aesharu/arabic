// The four hundred words for talking to Dima (public/js/data/essentials.js) and the Saudi voice files made from them.
//
...
import { ESSENTIALS, GROUPS, wordOf } from "../public/js/data/essentials.js";
...
test("four hundred words, in groups, both languages, one file each", () => {
  assert.equal(ESSENTIALS.length, 400);
  assert.equal(GROUPS.length, 34);
  assert.equal(new Set(ESSENTIALS.map(w => w.file)).size, 400, "two words share a file name");
...
test("every word comes from the plan or from her", () => {
  ...
  const strays = ESSENTIALS.filter(w => !known.has(plain(w.ar))).map(w => w.ar);
...
test("every consonant is marked — no letter is left for the voice to guess", () => {
  ...
  const bad = [];
  for (const w of ESSENTIALS) {
    for (const token of w.said.split(/\s+/).map(x => x.replace(/[؟?!.,،…]+$/, ""))) {
      const chars = [...token];
      chars.forEach((c, i) => {
        ...
        // the ل of الـ, with or without a prefix and whatever mark the alif carries
        if (c === "ل" && chars.slice(0, i).reverse().find(x => !MARK.test(x)) === "ا") return;
...
test("the Arabic and the pronunciation are the same word, letter for letter", () => {
  const bad = ESSENTIALS.filter(w => !sameWord(w.ar, w.say)).map(w => `${w.ar} / ${w.say}`);
...
test("no word ends in -ak: he is always speaking to her", () => {
  for (const w of ESSENTIALS) {
    if (!/\byou(r|'re)?\b/i.test(w.en)) continue;        // سمك is fish, not "your"
...
// Every word that has a Saudi voice really has its file, and the ones still waiting for a voice are
// exactly the ones data/voices.js says are waiting — so a word can never quietly point at a file that
// isn't there, and the waiting list can never drift away from what the folder actually holds.
test("every word with a voice has its file, the waiting list matches, and the page is named in both languages", async () => {
  const { VOICE_WAITING } = await import("../public/js/data/voices.js");
  const dir = new URL("../public/audio/saudi/", import.meta.url);
  const waiting = new Set(VOICE_WAITING);
  const missing = ESSENTIALS.filter(w => !existsSync(new URL(w.file + ".mp3", dir))).map(w => w.file);
  assert.deepEqual(missing.filter(f => !waiting.has(f)), [], "a word has no voice file and isn't on the waiting list");
  assert.deepEqual([...waiting].filter(f => existsSync(new URL(f + ".mp3", dir))), [],
    "a word on the waiting list has its voice file now — take it off the list in data/voices.js");
  for (const w of ESSENTIALS)
    if (!waiting.has(w.file)) assert.ok(statSync(new URL(w.file + ".mp3", dir)).size > 8000, `${w.file}.mp3 is too small to be a word`);
  for (const k of ["nav.essentials", "essentials.title", "essentials.sub", "essentials.howTitle", "essentials.how", "essentials.count"])

...
// and every verb has all its forms, meanings in English, and pronunciation.
...
test("every verb: all eight people in three tenses, meanings, pronunciation, unique ids", () => {
  ...
  for (const v of VERBS) {
    for (const f of ["en", "enPast"]) assert.ok(v[f], `${v.id}: ${f}`);
    const c = conjugate(v);
    ...
    if (v.note) for (const l of ["en", "najdi"]) assert.ok(v.note[l], `${v.id}: note ${l}`);

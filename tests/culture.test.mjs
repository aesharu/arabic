// Culture cards (public/js/data/culture.js) and page scenes (public/js/core/scenes.js): every story in both languages,
// on pages that exist, with a picture that exists and a word with pronunciation and English.
...
test("culture stories: both languages, pages and pictures that exist, a word to hear", () => {
  assert.ok(CULTURE.length >= 15);
  assert.equal(new Set(CULTURE.map(c => c.id)).size, CULTURE.length);
  for (const c of CULTURE) {
    for (const l of ["en", "najdi"]) assert.ok(c.title[l] && c.text[l], `${c.id}: ${l}`);
    ...
    const [ar, say, en] = c.word;
    assert.match(ar, /[؀-ۿ]/);
    assert.ok(say && en, `${c.id}: word`);
  }
});

test("every scene has its word in both languages", () => {
